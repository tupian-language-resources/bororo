...
class Dataset(BaseDataset):
    ...
    def cmd_download(self, args):
        ...
        self.raw_dir.download(
            "https://lingulist.de/edictor/triples/get_data.py?file=bororo&remote_dbase=bororo.sqlite3",
            "bororo.tsv"
        )
        print('... bororo.tsv done')
        subprocess.check_call(
            'git -C {} submodule update --remote'.format(self.dir.resolve()), shell=True)
        print('... sources.bib done')
        fetch_sheet('bororo_languages', output=self.etc_dir / 'languages.tsv')
        fetch_sheet('bororo_concepts', output=self.etc_dir / 'concepts.tsv')
...
#                       SubGroup=row['SubGroup'],
        ...
        wl = lingpy.Wordlist(self.raw_dir.joinpath('bororo.tsv').as_posix())
        ...
        with open(self.dir.joinpath('errors.md'), 'w', encoding="utf-8") as f:
            f.write('# Error Analysis for Bororo\n')
